...
local Library do 
    ...
    local Utility = {} do
        ...
        function Utility:Round(Number, Decimals)
            ...
            return MathFloor(Number * Shift + 0.5) / Shift
        end

        function Utility:FormatNumber(Number)
            if Number >= 1000000 then
                return StringFormat("%.1fM", Number / 1000000)
            elseif Number >= 1000 then
                return StringFormat("%.1fK", Number / 1000)
            else
                return tostring(MathFloor(Number))
            end
        end
        ...
        function Utility:GetTextBounds(Text, Font, Size)
            ...
            return TextService:GetTextSize(Text, Size.Y, Font, Vector2New(Size.X, math.huge))
        end

        function Utility:Connection(Signal, Callback)
            local Connection = Signal:Connect(Callback)
            TableInsert(AdvancedLibrary.Connections, Connection)
            return Connection
        end
        ...
        function Utility:Disconnect(Connection)
            if Connection then
                ...
                local Index = TableFind(AdvancedLibrary.Connections, Connection)
                if Index then
                    TableRemove(AdvancedLibrary.Connections, Index)
                end
            end
        end
        ...
        function Utility:Thread(Function)
            ...
            TableInsert(AdvancedLibrary.Threads, Thread)
        ...
        function Utility:ColorLerp(Color1, Color2, T)
            return Color1:Lerp(Color2, T)
        end

        function Utility:MakeGradient(Color1, Color2, Rotation)
            Rotation = Rotation or 90
            return Utility:Create("UIGradient")({
                Color = RGBSequence({
                    RGBSequenceKeypoint(0, Color1),
                    RGBSequenceKeypoint(1, Color2)
                }),
                Rotation = Rotation
            })
        end
        ...
        function Utility:MakeStroke(Color, Thickness, Transparency)
            return Utility:Create("UIStroke")({
                Color = Color or AdvancedLibrary.Theme["Border"],
        ...
        function Utility:MakePadding(Left, Right, Top, Bottom)
            return Utility:Create("UIPadding")({
                PaddingLeft = UDimNew(0, Left or 0),
                PaddingRight = UDimNew(0, Right or 0),
                PaddingTop = UDimNew(0, Top or 0),
                PaddingBottom = UDimNew(0, Bottom or 0)
            })
        end

        function Utility:Ripple(Object, Position, Color)
            local Ripple = Utility:Create("Frame")({
                BackgroundColor3 = Color or AdvancedLibrary.Theme["Accent"],
                BackgroundTransparency = 0.5,
                BorderSizePixel = 0,
                Position = UDim2New(Position.X.Scale, Position.X.Offset, Position.Y.Scale, Position.Y.Offset),
                Size = UDim2New(0, 0, 0, 0),
                AnchorPoint = Vector2New(0.5, 0.5),
                ZIndex = Object.ZIndex + 1,
                Parent = Object
            }, {
                Utility:MakeCorner(9999)
            })

            local Size = MathMax(Object.AbsoluteSize.X, Object.AbsoluteSize.Y)
            
            TweenService:Create(Ripple, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = UDim2New(0, Size * 2, 0, Size * 2),
        ...
        function Utility:PlaySound(SoundId)
            if not AdvancedLibrary.Features.NotificationSound then return end
        ...
        function PerformanceMonitor:Start()
            ...
            Utility:Connection(RunService.RenderStepped, function()
                ...
                if CurrentTime - LastUpdate >= 1 then
                    AdvancedLibrary.Performance.FPS = FrameCount
                    FrameCount = 0
                    LastUpdate = CurrentTime
                    
                    -- Update memory
                    AdvancedLibrary.Performance.Memory = gcinfo()
                    
                    -- Calculate ping
                    local Success, Ping = pcall(function()
                        return LocalPlayer:GetNetworkPing() * 1000
                    end)
                    AdvancedLibrary.Performance.Ping = Success and Ping or 0
                end
            end)
        end

        function PerformanceMonitor:GetFPS()
            return AdvancedLibrary.Performance.FPS
        end

        function PerformanceMonitor:GetPing()
            return MathFloor(AdvancedLibrary.Performance.Ping)
        end

        function PerformanceMonitor:GetMemory()
            return Utility:FormatNumber(AdvancedLibrary.Performance.Memory)
        end
    end

    -- Rainbow Mode
    local RainbowMode = {} do
        local RainbowHue = 0

        function RainbowMode:Start()
            if AdvancedLibrary.Features.RainbowMode then
                Utility:Connection(RunService.RenderStepped, function()
                    RainbowHue = (RainbowHue + 0.005) % 1
                    local RainbowColor = FromHSV(RainbowHue, 1, 1)
                    
                    for _, Item in AdvancedLibrary.ThemeItems do
                        if Item.RainbowEnabled then
                            Item.Instance.BackgroundColor3 = RainbowColor
                        end
                    end
                end)
            end
        ...
        function RainbowMode:Toggle(State)
            AdvancedLibrary.Features.RainbowMode = State
            if State then
                self:Start()
            end
        end
    ...
    Parent = Library.NotifHolder.Instance,
    Name = "\0",
    Size = UDim2New(0, 200, 0, 30),  -- SỬA: Thêm chiều rộng tối đa 250px
    BorderColor3 = FromRGB(0, 0, 0),
    BorderSizePixel = 0,
    AutomaticSize = Enum.AutomaticSize.Y,  -- SỬA: Chỉ tự động theo chiều cao
    BackgroundColor3 = FromRGB(16, 18, 18)
    ...
    Text = Name,
    AnchorPoint = Vector2New(0, 0.5),
    Size = UDim2New(1, Icon and -24 or 0, 0, 15),  -- SỬA: Chiều rộng = 100% - icon
    BackgroundTransparency = 1,
    Position = UDim2New(0, Icon and 24 or 0, 0.5, 0),
    BorderSizePixel = 0,
    AutomaticSize = Enum.AutomaticSize.Y,  -- SỬA: Chỉ tự động theo chiều cao
    TextSize = 14,
    TextWrapped = true,  -- THÊM: Cho phép text xuống dòng
    TextXAlignment = Enum.TextXAlignment.Left,  -- THÊM: Căn trái
    BackgroundColor3 = FromRGB(255, 255, 255)
})
            end
